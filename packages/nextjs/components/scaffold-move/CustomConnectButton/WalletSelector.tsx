    ...
    <>
      ...
      <AboutAptosConnect renderEducationScreen={renderEducationScreens}>
        {walletSelectorModalOpen && (
          <dialog open className="modal absolute z-100 w-screen h-screen text-background modal-open">
            <div className="modal-box relative z-100 max-w-sm border-2 border-base-300 shadow-xl">
              {/* Modal header */}
                  ...
                  <div className="flex flex-col gap-3">
                    {
                      availableWallets
                        .sort((a, b) => {
                          const priority = ["Razor Wallet", "Nightly", "Leap Wallet", "Pontem Wallet"];
                          const aIndex = priority.indexOf(a.name);
                          const bIndex = priority.indexOf(b.name);
                          if (aIndex === -1 && bIndex === -1) return 0; // Keep original order for others
                          if (aIndex === -1) return 1; // Push non-priority to the end
                          if (bIndex === -1) return -1;
                          return aIndex - bIndex; // Sort priority wallets
                        })
                        .map(wallet => (
